<template>
  <div>
    ...
    <div style="margin: 10px 0">
      <el-input style="width: 200px" suffix-icon="el-icon-search" placeholder="请输入ID" v-model="id"></el-input>
      <el-input style="width: 200px" suffix-icon="el-icon-message" placeholder="请输入区域" class="ml-5" v-model="area"></el-input>
      <el-input style="width: 200px" suffix-icon="el-icon-position" placeholder="请输入关联切片ID" class="ml-5" v-model="connSlicing"></el-input>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="headerBg" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="id" label="MEC Id" width="80" align="center">
        <template slot-scope="scope">
          mec-{{scope.row.id}}
        </template>
      </el-table-column>
      <el-table-column prop="area" label="MEC所属片区" width="120" align="center"></el-table-column>
    ...
    <el-dialog title="MEC信息" :visible.sync="dialogFormVisible" width="30%" >
      <el-form label-width="80px" size="small" style="margin-right: 30px">
        ...
        <el-form-item label="片区" >
          <el-select v-model="form.area" autocomplete="off" placeholder="请选择MEC片区" style="width: 100%">
            <el-option v-for="item in areaOptions" :value="item.label" :key="item.value" :label="item.label">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="是否共享" >
          <el-select v-model="form.share" autocomplete="off" placeholder="是否共享" style="width: 100%">
            <el-option v-for="item in shareOptions" :value="item.label" :key="item.value" :label="item.label">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    save(){
      this.request.post("mec", this.form).then(res=>{
        if(res){
    ...
    handleDelete(id){
      this.request.delete("mec/"+id).then(res=>{
        if(res){
