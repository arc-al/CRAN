...
import com.example.cran.simulation.Config.GetRunStatusAndRuntimeServlet;
...
//		System.setOut(blah.getPs());
        ...
        BreakLabel: while (vUnitCounter < ConfigJavaController.vSamplingTime.length && !vIsSuspend) {
            ...
            Optimization: while (vSubUniteCounter < ConfigJavaController.vSamplingNum && !vIsSuspend) {
                if (vIsSuspend)
                    continue BreakLabel;
                /** 跳出循环监测点 */
                fProgress = (double) ((vUnitCounter) * ConfigJavaController.vSamplingNum + vSubUniteCounter + 1)
                        / (double) (ConfigJavaController.vSamplingTime.length * (ConfigJavaController.vSamplingNum));
...
//				run3.textField_4.setText(Integer.toString(vSubUniteCounter));	//运行状态的第二个文本框
                ConfigJavaController.vUnitCounter=vUnitCounter;
                ConfigJavaController.vSubUniteCounter=vSubUniteCounter;
                GetRunStatusAndRuntimeServlet.vSubUniteCounter=vSubUniteCounter;

                if (!ConfigJavaController.vFlagPause) { // 如果当前处于暂停状态（false表示处于暂停状态）
                    pauseTime += System.currentTimeMillis() - endTime; // 更新pauseTime
                }
                endTime = System.currentTimeMillis();
                if (ConfigJavaController.vFlagPause) { // 如果当前处于在运行状态
...
//					run3.textField_3.setText(sdf.format(new Date(endTime - run3.startTime - pauseTime)));	//运行时间
                    ...
                    try {
                        ...
                        if (vOptimizationStart) {
                            System.out.println(
                                    "--优化后的结果--，当前时段信息-------" + ConfigJavaController.vSamplingTime[vUnitCounter]
                                            + "----当前的时刻-----" + vSubUniteCounter);
                        } else { // 只执行这个分支
                            System.out.println(
                                    "--未优化的结果--，当前时段信息-------" + ConfigJavaController.vSamplingTime[vUnitCounter]
                                            + "----当前的时刻-----" + vSubUniteCounter);
                        }
                        //进行相关配置
